...
public class LibraryManagerJFrame extends javax.swing.JFrame {
    ...
    private void rentalSubmitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_rentalSubmitActionPerformed
        // TODO add your handling code here:
        String rentalReq = String.valueOf(rentalReqId.getSelectedItem());

        CustomerRequest cr = this.branch.getLibrary().getCustomerRequest(rentalReq);

        cr.setIsAccepcted(true);

        rentalReqId.setModel(getRentalReqModel());
        rentalReqId.repaint();
        rentalReqTable.setModel(getRentalReqTableModel());
        rentalReqTable.repaint();
        magazineTable.setModel(getMagazineModel());
        magazineTable.repaint();
        this.bookTable.setModel(getBooksModel());
        this.bookTable.repaint();
